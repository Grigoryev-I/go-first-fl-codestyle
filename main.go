...
func attack(name, class string) string {
	switch class {
	case "warrior":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", name, 5+random(3, 5))
	case "mage":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", name, 5+random(5, 10))
	case "healer":
		return fmt.Sprintf("%s нанес урон противнику равный %d.", name, 5+random(-3, -1))
	default:
...
func defence(name, class string) string {
	switch class {
	case "warrior":
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+random(5, 10))
	case "mage":
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+random(-2, 2))
	case "healer":
		return fmt.Sprintf("%s блокировал %d урона.", name, 10+random(2, 5))
	default:
		return "неизвестный класс персонажа"
	}
}
